...
\subsection{Data representation and preprocessing}\label{sec:preproc}
...
\subsection{Training and evaluation protocol}\label{sec:eval}
Optimization uses AdamW with learning rate $2\times10^{-4}$, $(\beta_1,\beta_2)=(0.9,0.999)$, cosine learning-rate decay, 200 warmup steps, and gradient clipping with max norm 16. Training uses mixed precision (FP16), an effective batch size of 32, and 50 epochs per fold. Multi-GPU execution is handled through HuggingFace Accelerate\footnote{\texttt{https://huggingface.co/docs/accelerate/en/index}}.
...
To quantify the complementary burden of over-detection under the same IoU matching rule, we additionally report false positives per scan: the mean number of predicted connected components (also after the 50-voxel size filter) that are not matched to any annotated tumor at IoU $\ge 0.5$. This metric characterizes spurious lesion calls and downstream review load, but is interpreted as a secondary deployment-oriented endpoint relative to sensitivity. For voxel-level overlap, we report Dice Similarity Coefficient (DSC) within the physics-informed tumor-candidate region ($\mathrm{cand}$), i.e., voxels flagged by the HU-based preprocessing pipeline in Section~\ref{sec:preproc}, rather than over the entire breast mask. All metrics are computed per fold and summarized as fold-wise mean $\pm$ standard deviation.

\section{Results}
\subsection{Experimental setup}
Results are reported on five-fold patient-level cross-validation with identical split logic for all methods; for each fold, models are trained from scratch and evaluated on the held-out patient subset. Using the metric definitions in Section~\ref{sec:eval}, we summarize candidate-region DSC, tumor detection sensitivity, and false positives per scan, reported as fold-wise mean $\pm$ standard deviation. We additionally report deployment-relevant computational characteristics, including the fixed 25-step Euler schedule and wall-clock prediction time per scan under the same hardware environment. This section presents observations only; interpretation is deferred to the Discussion.

\subsection{Comparison with deterministic baseline}\label{sec:comparison}
...
\begin{tabular}{lccc}
\hline
Method & DSC $\uparrow$ & Sensitivity (\%) $\uparrow$ & FP/scan $\downarrow$ \\
\hline
...
Per-fold values are stable, without fold-specific outliers that invert the method ordering on overlap metrics. Inference cost is summarized by the fixed 25-step Euler integration and measured runtime per scan under the same execution environment for both methods.

\subsection{Ablation study: augmentation efficacy}\label{sec:aug_ablation}
To quantify the role of ROI-centered 3D patch augmentation, we ablate geometric components while holding all other training settings fixed. Table~\ref{tab:aug_ablation} reports four variants on fold~0 only (augmentation ablation; main results in Table~\ref{tab:main_comparison} remain five-fold): no augmentation, scaling only, scaling plus flipping (without rotation), and full augmentation (scaling, rotation, and flipping). Candidate-region DSC, detection sensitivity, and false positives per scan are evaluated under the same protocol as Table~\ref{tab:main_comparison}.

\begin{table}
\caption{\label{tab:aug_ablation}Ablation of ROI-centered 3D patch augmentation components on fold~0. Metrics follow Table~\ref{tab:main_comparison}: candidate-region DSC at prediction threshold 0.3, IoU $\ge 0.5$ lesion matching (minimum lesion size 50 voxels), detection sensitivity, and false positives per scan.}
\begin{center}
\begin{tabular}{lccc}
\hline
Augmentation variant & DSC $\uparrow$ & Sensitivity (\%) $\uparrow$ & FP/scan $\downarrow$ \\
\hline
No augmentation & 0.407 & 37.5 & 1.625 \\
Scale only & 0.435 & 50.0 & 0.625 \\
Scale + flip & \textbf{0.545} & 62.5 & 1.000 \\
Full (scale + rotate + flip) & 0.544 & \textbf{75.0} & \textbf{0.375} \\
\hline
\end{tabular}
\end{center}
\end{table}

Removing augmentation leads to unstable optimization and the weakest endpoint performance (candidate-region DSC 0.407; detection sensitivity 37.5\%; 1.625 false positives per scan). Scaling alone improves overlap and detection (0.435; 50.0\%) while reducing false positives to 0.625 per scan, indicating that ROI-centered scale jitter is beneficial for this small-cohort, sparse-foreground setting. Adding flipping on top of scaling yields the largest gain in candidate-region DSC (0.545) at a modest false-positive cost (1.000 per scan), showing that mirror symmetry is the dominant geometric prior for voxel-level overlap in axis-aligned breast CT. The full configuration matches this DSC level (0.544) while further improving lesion-level detection sensitivity to 75.0\% (6/8 test lesions matched at IoU $\ge 0.5$) and lowering false positives to 0.375 per scan, suggesting that rotation---when combined with flipping---acts primarily as a complementary regularizer for recall and specificity rather than boundary refinement. We therefore adopt full augmentation in all main experiments.
\subsection{Ablation study: transport formulation}\label{sec:transport_ablation}
...
\begin{tabular}{lccc}
\hline
Transport formulation & DSC $\uparrow$ & Sensitivity (\%) $\uparrow$ & FP/scan $\downarrow$ \\
\hline
...
\subsection{Ablation study: ODE integration steps}\label{sec:ode_steps}
...
\subsection{Size-stratified detection analysis}\label{sec:size}
...
\subsection{Detection operating characteristics}\label{sec:operating}
...
\subsection{Boundary versus interior agreement}\label{sec:boundary}
...
Figure~\ref{fig:qualitative_results} presents representative qualitative cases comparing deterministic UNet and Flow-Matching outputs on lesion-centered SBCT slices. The selected examples span three operating regimes observed in the quantitative analysis: a high-overlap large lesion, a boundary-disagreement case, and a very small lesion close to the 50-voxel evaluation floor.
...
From a computational-physics viewpoint, the choice of Flow-Matching over diffusion is also practical for high-resolution BCT deployment: relative to diffusion-style segmentation, the same probabilistic framing is retained while inference remains tractable through fixed-step ODE integration \cite{lipman2023flow, han2024semflow, zhao2025maisi}. Sliding-window inference over large 3D volumes requires evaluating many patches, and per-patch integration cost scales directly with the number of function evaluations (NFE). A diffusion-style sampler with 200 steps can be computationally prohibitive at volume level, whereas 25-step Euler integration reduces NFE by approximately an order of magnitude while preserving conditional generation quality in this setting \cite{lipman2023flow, han2024semflow, zhao2025maisi, liu2023flow}; the step-count ablation (Section~\ref{sec:ode_steps}) confirms that 25 steps lie near the observed performance plateau, providing a practical accuracy--cost compromise. In addition, ODE-based integration is deterministic for a fixed input state, which supports reproducibility requirements in clinical and regulatory contexts.
...
\section{Conclusion}
This study indicates that generative segmentation with Flow-Matching is a practical option for high-resolution BCT in the small-cohort regime. The central empirical message is a consistent candidate-region overlap gain together with a marked reduction in false positives per scan, while lesion-level detection sensitivity remains comparable to a deterministic UNet baseline; this overlap gain is non-trivial given the small annotated cohort, the intrinsically low tumor-to-parenchyma contrast, and a high-resolution photon-counting modality on which conventional-CT models do not transfer directly. The study also reveals that training behavior in this regime depends critically on how data variation is constructed: ROI-centered 3D geometric perturbation functions as a convergence condition under extreme class imbalance, not merely a minor performance enhancement. In addition, the residual performance gap is concentrated in small, low-contrast lesions, indicating that the dominant limitation in this cohort is lesion-level learning and detection rather than contour refinement. Taken together, these findings imply that progress in BCT segmentation is best framed as a joint modeling-and-physics problem, where the value lies in coupling probabilistic inference with modality-specific priors---HU-based tumor-candidate localization and the partial-volume mixing that makes tumor boundaries physically ambiguous---so that algorithmic outputs remain consistent with known image ambiguity. More broadly, the results support a shift from pure overlap optimization toward evaluation strategies that emphasize missed-tumor rate (via detection sensitivity) and, secondarily, false-positive burden. At the same time, the present evidence is single-center and feasibility-oriented; deployment-level claims require the broader validation outlined in the Discussion. 
