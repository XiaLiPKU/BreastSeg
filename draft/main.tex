...
\end{equation}
The resulting final latent state $z_1$ is subsequently decoded back to the voxel space using the VAE decoder. A threshold of 0.5 is applied to the continuous voxel predictions to yield the binary tumor segmentation mask. Finally, overlapping local ROI patches are assembled into a full-volume prediction by averaging the overlapping regions.
...
Model quality is evaluated with voxel-level overlap and tumor-level detection metrics. We report Dice Similarity Coefficient (DSC), tumor detection sensitivity (fraction of tumors detected above a 0.5 in IoU), and false-positive behavior at connected-component level. Metrics are computed per fold and summarized across five folds.
...
Scale only &  &  & Yes \\
Scale + rotate &  &  & Yes \\
Full (scale + rotate + flip) & \textbf{X} & \textbf{X} & Yes \\
\hline
\end{tabular}
\end{center}
\end{table}
