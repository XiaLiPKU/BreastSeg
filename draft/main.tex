...
Because malignant tumors in BCT are typically hyperdense relative to surrounding dense parenchyma \cite{weber2024potential}, we restrict the search space using HU-based candidate masks. Let $I(x)$ denote the input volume and $\tau_{\mathrm{HU}}$ a fixed candidate threshold. Candidate generation applies HU thresholding first, followed by morphological erosion and then dilation before connected-component extraction. This erosion--dilation step suppresses thin, line-like glandular tissue candidates and retains more lesion-focused regions. Each connected component is then converted to an ROI box:
\begin{equation}
